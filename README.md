...
### 6. 工作流框架
- **功能说明**：提供多模式工作流控制系统，支持不同的工作模式切换
- **主要工作模式**：
  - **调试模式 (DebugMode)**：用于系统调试，单独控制舵机位置和激光器状态
  - **C模式 (CMode)**：按预设掩码控制激光器开关，配合舵机连续扫描
  - **D模式 (DMode)**：完整的测量模式，按设定的角度间隔控制舵机运动，并循环切换激光器
- **数据上传**：支持周期性数据上传，包含ADC、SDADC、自动增益调节数据
- **控制参数**：通过串口可配置多种工作参数
  - 数据上传频率
  - 自动增益调节频率
  - 舵机转动参数
...
- **控制命令处理**：
  - `setCtrlParams()` - 处理控制参数设置命令
  - 支持读取/写入控制参数、设置工作频率、设置工作模式等命令
- **命令类型**：
  - 0x01 - 读取控制参数
  - 0x02 - 批量写入控制参数
  - 0x03 - 设置串口发送频率
...
#### 数据上传功能
- **功能说明**：将采集的数据按规定格式打包上传到上位机
- **数据帧结构**：
  - 帧头：2字节 (0xA9, 0xB5)
  - ADC数据：8字节 (4通道×2字节)
  - SDADC数据：16字节 (8通道×2字节)
  - 自动增益调节数据：8字节 (8通道×1字节)
  - 帧序号信息：4字节 (主帧2字节，子帧2字节)
  - 激光器状态：1字节
  - 帧尾：1字节 (0x33)
- **使用方法**：
  ```c
...
4. **扩展工作模式**：
   - 在现有的工作模式基础上添加新的模式函数
   - 在`main()`循环中添加模式检测逻辑
   - 通过`modeInit()`函数初始化新模式参数

5. **自动增益调节功能扩展**：
   - 增加更多的可调增益通道
   - 修改`adjaddr`数组添加新设备地址
   - 在`autoadj`数组中添加对应的数据位
...
│   └── usart.h                    // 串口模块头文件
├── Src/                           // 源文件目录
│   ├── main.c                     // 主程序，系统初始化和主循环，包含工作流框架实现
│   ├── adc.c                      // ADC模块实现
│   ├── dma.c                      // DMA模块实现
│   ├── fashion_driver.c           // 舵机驱动实现
│   ├── fashion_driver_example.c   // 舵机驱动使用示例
│   ├── gpio.c                     // GPIO模块实现
│   ├── i2c.c                      // I2C模块实现，包含自动增益调节功能
│   ├── sdadc.c                    // SDADC模块实现
│   ├── tim.c                      // 定时器模块实现，用于工作流定时控制
│   └── usart.c                    // 串口模块实现
```
...
- **main.c**：包含完整的工作流框架实现
  ...
  - `dModeSet()` - D模式实现
  - `modeInit()` - 模式初始化函数
  - `setCtrlParams()` - 控制参数处理函数

- **i2c.c**：IIC驱动实现，支持自动增益调节
  - 包含IIC通信配置和DMA传输
  - 实现IIC中断回调处理
