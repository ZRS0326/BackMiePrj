...
uint8_t mask_lidar[4] = {0x00, 0x01, 0x02,
                         0x03}; // 00 01 10 11 ....111 000当前只有两个激光器
uint8_t index_lidar = 0;        // 激光器开启状态
uint8_t dmode_possize = 0;      // dMode的测量点数
int16_t dmode_pos[10] = {};     // dMode的测量点数组（角度）
...
int main(void) {
  ...
  uartCtrl.flagMask = CHE | DebugMode | SPMode; // 模式控制掩码
  uartCtrl.posLow = 0;
  uartCtrl.posHigh = 500;
  uartCtrl.posSet = 0;
  uartCtrl.adjTime = 10;
  uartCtrl.uartUploadTime = 1;
  uartCtrl.fashionTime = 1000;
  uartCtrl.lidarTime = 10;
  uartCtrl.delayTime = 50;
  /* USER CODE END 1 */
...
void setCtrlParams(void) {
  if (recv_frame2[0] == 0xA0 && recv_frame2[1] == 0xB3) {
    ...
    case 0x23: // 带参数启动dMode
      uartCtrl.flagMask = (recv_frame2[3] << 8) + recv_frame2[4];
      uartCtrl.fashionTime = (recv_frame2[5] << 8) + recv_frame2[6];
      uartCtrl.lidarTime = (recv_frame2[7] << 8) + recv_frame2[8];
      uartCtrl.delayTime = (recv_frame2[9] << 8) + recv_frame2[10];
      dmode_possize = recv_frame2[11];
      for (uint8_t i = 0; i < dmode_possize; ++i) {
        dmode_pos[i] = (recv_frame2[12 + i * 2] << 8) + recv_frame2[13 + i * 2];
      }
      modeInit();
      break;
    case 0x24: // 带参数启动dMode
      uartCtrl.flagMask = 0x0008;
      uartCtrl.lidarTime = (recv_frame2[5] << 8) + recv_frame2[6];
      uartCtrl.delayTime = (recv_frame2[7] << 8) + recv_frame2[8];
      modeInit();
      break;
      // 后续添加其他指令
    default:
...
void dataUpload(void) {
  if (uartCtrl.flagMask & SPMode) {
    // CH1[0,4,0,4],CH2[2,1,1,5],CH3[3,7,2,6],CH4[5,6,3,7]
    char buffer[100];
    int slen = 0;
    int V0, V1;
    // 新增两个数，一个是dmode_pos[index],一个是flagMask二进制
    slen = sprintf(buffer, "%04x,%d,", uartCtrl.flagMask, uartCtrl.posSet);
    // 检查启用的通道并输出对应的值
    if (uartCtrl.flagMask & CHE) {
      // CH1对应sdadc_frame[0], sdadc_frame[4], autoadj[0], autoadj[4]
      V0 = (sdadc_frame[0] + 32767) * 3300.0 / 65535.0;
      V1 = (sdadc_frame[4] + 32767) * 3300.0 / 65535.0;
      slen += sprintf(buffer + slen, "%d,%d,%d,%d,", V0, V1, autoadj[0],
                      autoadj[4]);
    }

    if (uartCtrl.flagMask & CHS) {
      // CH2对应sdadc_frame[2], sdadc_frame[1], autoadj[1], autoadj[5]
      V0 = (sdadc_frame[2] + 32767) * 3300.0 / 65535.0;
      V1 = (sdadc_frame[1] + 32767) * 3300.0 / 65535.0;
      slen += sprintf(buffer + slen, "%d,%d,%d,%d,", V0, V1, autoadj[1],
                      autoadj[5]);
    }

    if (uartCtrl.flagMask & CHN) {
      // CH3对应sdadc_frame[3], sdadc_frame[7], autoadj[2], autoadj[6]
      V0 = (sdadc_frame[3] + 32767) * 3300.0 / 65535.0;
      V1 = (sdadc_frame[7] + 32767) * 3300.0 / 65535.0;
      slen += sprintf(buffer + slen, "%d,%d,%d,%d,", V0, V1, autoadj[2],
                      autoadj[6]);
    }

    if (uartCtrl.flagMask & CHW) {
      // CH4对应sdadc_frame[5], sdadc_frame[6], autoadj[3], autoadj[7]
      V0 = (sdadc_frame[5] + 32767) * 3300.0 / 65535.0;
      V1 = (sdadc_frame[6] + 32767) * 3300.0 / 65535.0;
      slen += sprintf(buffer + slen, "%d,%d,%d,%d,", V0, V1, autoadj[3],
                      autoadj[7]);
    }
  ...
  memcpy(&data_frame_upload[2], sdadc_frame,
         sizeof(sdadc_frame)); // sdadc 8*2 = 16Bytes
  memcpy(&data_frame_upload[18], autoadj,
         sizeof(autoadj)); // autoadj 8*1 = 8Bytes
  memcpy(&data_frame_upload[26], &uartCtrl.flagMask, sizeof(uartCtrl.flagMask));
  memcpy(&data_frame_upload[28], &uartCtrl.posSet, sizeof(uartCtrl.posSet));
  HAL_UART_Transmit_IT(&huart2, data_frame_upload, 30);
  // 0-1 		帧头0xA9 0xB5 1*2 2-9 		4ch adc
  // 4*2 10-25 	8ch sdadc 8*2 26-33		8ch autoadj
  // 8*1 34-37		2ch frame(master/slave)	2*2 38			1ch
  // lidar state					1*1 39
  // 帧尾0x33								1*1
  //		memcpy(&data_frame_upload[2],adc_frame,sizeof(adc_frame));
  ////adc 4*2 = 8Bytes
  //		memcpy(&data_frame_upload[10],sdadc_frame,sizeof(sdadc_frame));//sdadc
  // 8*2 = 16Bytes
  //		memcpy(&data_frame_upload[26],autoadj,sizeof(autoadj));//autoadj
  // 8*1 = 8Bytes
...
void debugModeSet() {
  if (uartCtrl.flagMask & DebugMode) {
    ...
    HAL_Delay(uartCtrl.fashionTime + uartCtrl.delayTime);
...
void cModeSet() {
  if (uartCtrl.flagMask & CMode) {
    // 初始化
    fashion_sync_set_angle(uartCtrl.posLow,
                           uartCtrl.fashionTime); // 启动舵机到初始位置posLow
    ...
    while (uartCtrl.flagMask & CMode) {
      ...
      // 切换舵机目标位置
      target = (abs(angle_read - uartCtrl.posLow) <= 10) ? uartCtrl.posHigh
                                                         : uartCtrl.posLow;
...
void dModeSet() {
  if (uartCtrl.flagMask & DMode) {
    ...
    while (uartCtrl.flagMask & DMode) {
      ...
      fashion_sync_set_angle(dmode_pos[index], uartCtrl.fashionTime);
      HAL_Delay(uartCtrl.fashionTime + uartCtrl.delayTime);
      uartCtrl.posSet = dmode_pos[index];
      HAL_TIM_Base_Start_IT(&htim4); // 启动定时器发送数据
      HAL_Delay(uartCtrl.lidarTime); // 采样数据时长
      HAL_TIM_Base_Stop_IT(&htim4);  // 启动定时器发送数据
      if (direction == 1) {
        if (index == dmode_possize - 1) {
          index = dmode_possize - 2;
          direction = 0;
        } else {
          ++index;
        }
...
void fModeSet(void) {
  ...
  char buffer[200];
  int slen = 0;
  while (uartCtrl.flagMask == 0x0008) {
		slen = 0;
    // 循环开启每个方向的前两个端口进行采样
    for (int i = 0; i < 8; ++i) {
      // 根据索引确定端口
      ...
      case 5:
        ...
        break; // N2
      case 6:
        HAL_GPIO_WritePin(W1_GPIO_Port, W1_Pin, 1);
        break; // W1
      case 7:
        HAL_GPIO_WritePin(W2_GPIO_Port, W2_Pin, 1);
        break; // W2
      }
      HAL_Delay(uartCtrl.lidarTime);
      data1[i] = (sdadc_frame[0] + 32767) * 3300.0 / 65535.0;
      data2[i] = (sdadc_frame[4] + 32767) * 3300.0 / 65535.0;
      slen += sprintf(buffer + slen, "%d,%d,", data1[i], data2[i]);
      // 关闭端口
      switch (i) {
      case 0:
      ...
      case 5:
        ...
        break; // N2
      case 6:
        HAL_GPIO_WritePin(W1_GPIO_Port, W1_Pin, 0);
        break; // W1
      case 7:
        HAL_GPIO_WritePin(W2_GPIO_Port, W2_Pin, 0);
        break; // W2
      }
    }
    // 添加换行符
...
void autoGainAdj(void) {
  ...
  //		V_ad_1[i] = (sdadc_frame[Arr_label[i]] + 32767) * V_ref / 65535;
  ////i, Arr_label[i] 		A_origin[i] = 1000 * V_ad_1[i] / ((256 -
  /// autoadj[i])
  //* 3.92); //i,i,2i+1, 电流单位是nA
  ...
  //				HAL_I2C_Master_Transmit_DMA(&hi2c1, adjaddr[i],
  // iicdata, 2);
  //		}
  ...
  // 1. 由SDADC的值转换为第一级放大电压V1
  V_ad_1[i] =
      (sdadc_frame[Arr_label[i]] + 32767) * V_ref / 65535; // i, Arr_label[i]
  A_origin[i] =
      1000 * V_ad_1[i] / ((256 - autoadj[i]) * 3.92); // i,i,2i+1, 电流单位是nA
